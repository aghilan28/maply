...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, onBackToLanding }) => {
  ...
  const handleGuestSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const cleanName = guestNameInput.trim() || 'Guest Explorer';
    const result = authService.loginAsGuest(cleanName);
    onLoginSuccess(result.user);
  };

  const handleFillDemoCreds = () => {
    setEmail('aghilan@maply.com');
  ...
  return (
    <div className="relative min-h-screen w-full bg-black text-white flex flex-col justify-between overflow-hidden font-sans selection:bg-blue-500 selection:text-white">
          ...
          <div className="w-full max-w-[420px] bg-[#0A1320]/45 backdrop-blur-3xl border border-white/20 rounded-[36px] p-8 sm:p-10 shadow-[0_30px_90px_rgba(0,0,0,0.7),inset_0_1px_0_rgba(255,255,255,0.2)] relative overflow-hidden transition-all duration-300">
            ...
            <div className="mt-3.5">
              <button
                ...
                className="w-full bg-white/8 hover:bg-white/14 border border-white/18 text-white font-medium py-3.5 px-6 rounded-full flex items-center justify-center space-x-2 text-sm transition-all duration-200 cursor-pointer shadow-sm hover:border-blue-400/40"
              >
      ...
      {/* GUEST NAME PROMPT OVERLAY MODAL */}
      {showGuestModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
          <div className="w-full max-w-md bg-[#0b1329] border border-white/20 rounded-3xl p-6 sm:p-8 shadow-2xl relative text-left">
            <h3 className="text-xl font-bold text-white mb-1">Explore as Guest</h3>
            <p className="text-xs text-slate-300 mb-5">
              Enter your name to personalize your Maply session header.
            </p>
            ...
            <form onSubmit={handleGuestSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    autoFocus
                    placeholder="e.g. Alex, Arjun, Guest Explorer"
                    value={guestNameInput}
                    onChange={(e) => setGuestNameInput(e.target.value)}
                    className="w-full bg-white/[0.06] border border-white/20 focus:border-blue-400 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/25"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-end space-x-3 pt-2">
                <button
                  ...
                  className="px-4 py-2.5 rounded-full text-xs font-medium text-slate-300 hover:text-white hover:bg-white/10 transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2.5 rounded-full text-xs font-semibold bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-500/30 transition-all"
                >
                  Explore Maply →
                </button>
              </div>
